...
exports.keystoneAuth = function (req, res, next) {
	...
	if (req.user.canAccessContent) {
		req.user.canAccessContent(req)
			.then(result => {
				return next();
			})
			.catch(err => {
				return noAccess(req, res, false);
			})
	} else {
		return next();
	}
...
const noAccess = (req, res, needSignIn) => {
	...
	return res.redirect(needSignIn === false ? `/${adminPath}` : keystone.get('signin url') + from);
};
