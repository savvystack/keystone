...
var doSignin = function (lookup, req, res, onSuccess, onFail) {
	...
	if (typeof lookup.email === 'string' && typeof lookup.password === 'string') {
		...
		User.model.findOne({ email: emailRegExp }).exec(function (err, user) {
			if (user) {
				if (!user.active) {
					return onFail(new Error('Not an active user'));
				}
				// Compatible with ESA's old password(SHA1 hashed)
				if (sha1(lookup.password) === user.password) {
...
exports.keystoneAuth = function (req, res, next) {
	// Savvy Stack: call user model to determine the accessibility
	if (req.user && req.user.canAccessKeystone && req.user.canAccessContent(req)) {
		return next();
	} else {
		return noAccess(req, res);
	}
};

const noAccess = (req, res) => {
	if (req.headers.accept === 'application/json') {
		...
		var regex = new RegExp('^\/' + adminPath + '\/?$', 'i');
		var from = regex.test(req.originalUrl) ? '' : '?from=' + req.originalUrl;
		return res.redirect(req.user ? `/${adminPath}` : keystone.get('signin url') + from);
	}
};
